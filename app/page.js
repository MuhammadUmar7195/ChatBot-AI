    ...
    <>
      <div className={`flex flex-col h-screen p-3 ${primary}`}>
        <div className="flex justify-between items-center mb-4">
          <h1 className={`text-2xl sm:text-3xl md:text-4xl font-semibold ${text}`}>CHATBOT</h1>
          <div>
            <label htmlFor="theme" className={`text-xs sm:text-sm ${text}`}>Theme: </label>
        ...
        <div className={`flex-1 overflow-y-auto ${secondary} p-2 sm:p-4 rounded-lg`}>
          {message.map((msg, i) => (
            <div key={i} className={`mb-2 sm:mb-4 ${msg.role === "user" ? "text-right" : "text-left"}`}>
              <div className={`inline-block p-2 sm:p-3 rounded-lg ${msg.role === "user" ? `${accent} text-white` : `${primary} ${text}`}`}>
                {msg.text}
              </div>
              <p className={`text-xs sm:text-sm ${text} mt-1`}>
        ...
        <div className="flex items-center border-gray-100">
          <input
            ...
            className={`flex-auto p-2 sm:p-4 rounded-l-md border shadow-sm focus:outline-none`}
          />
          <button
            onClick={handleSendMessage}
            className={`p-2 sm:p-4 ${accent} text-white rounded-r-md hover:bg-opacity-80 focus:outline-none`}
          >
            Send
          </button>
        </div>
